  ...
  socket.on('sendMessage', async ({ roomId, userId, username, content, fileData }) => {
    try {
      ...
      if (content && content.includes('@ai')) {
        ...
        // Use fileData from the message we just saved if not provided
        let attachedFile = fileData;
        if (!attachedFile && messageObj.fileUrl) {
          attachedFile = {
            url: messageObj.fileUrl,
            name: messageObj.fileName,
            type: messageObj.fileType,
            size: messageObj.fileSize
          };
        }
        
        if (attachedFile) {
          console.log(`📎 File attached to @ai message: ${attachedFile.name}, type: ${attachedFile.type}`);
        }
        ...
        try {
          ...
          if (attachedFile && attachedFile.url) {
            try {
              ...
              console.log(`📎 Checking file: ${attachedFile.name}, isImage: ${isImage}, isPDF: ${isPDF}`);
              
              if (isImage || isPDF) {
                // Read the file from uploads directory
                // Handle both relative and absolute paths
                let filePath = attachedFile.url;
                
                // If path starts with /uploads, prepend __dirname
                ...
                if (fs.existsSync(filePath)) {
                  ...
                  let mimeType = attachedFile.type;
                  if (!mimeType || mimeType === 'application/octet-stream') {
                    if (attachedFile.name?.toLowerCase().endsWith('.pdf')) {
                      mimeType = 'application/pdf';
                    } else if (attachedFile.name?.match(/\.(jpg|jpeg)$/i)) {
                      mimeType = 'image/jpeg';
                    } else if (attachedFile.name?.match(/\.png$/i)) {
                      mimeType = 'image/png';
                    } else if (attachedFile.name?.match(/\.gif$/i)) {
                      mimeType = 'image/gif';
                    } else if (attachedFile.name?.match(/\.webp$/i)) {
                      mimeType = 'image/webp';
                    }
                  }
                  
                  console.log(`📎 Processing file for AI: ${attachedFile.name}, type: ${mimeType}, size: ${base64Data.length} bytes`);
